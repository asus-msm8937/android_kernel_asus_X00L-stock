...
if (led->cdev.brightness ==255) {
	 if(strcmp(led->cdev.name, "red") == 0)
     {
		...
		aw2013_write(led, 0x10, 0x7f);//red
...
else  
{
  ...
  {
  /*bug 268672,267084 modify by diganyun 2017/07/02 optimize red green brightness 0 begin*/
     if(strcmp(led->cdev.name, "red") == 0)
       {
	    	aw2013_write(led, 0x04, 0x00);
	    	aw2013_write(led, 0x08, 0x00);
	    	aw2013_write(led, 0x10, 0x00);
	    	aw2013_write(led, 0x1c, 0x00);
	    	aw2013_read(led, 0x09, &val);
			aw2013_write(led, 0x09, (val&0xfe));
			aw2013_read(led, 0x07, &val);
     	...
     	
     	{
     		aw2013_write(led, 0x05, 0x00);
            aw2013_write(led, 0x08, 0x00);
            aw2013_write(led, 0x11, 0x00);
            aw2013_write(led, 0x1d, 0x00);
            aw2013_read(led, 0x09, &val);
            aw2013_write(led, 0x09, (val&0xfd));
	    	aw2013_read(led, 0x07, &val);
            aw2013_write(led, 0x07, (val&0xfd));
	}
	/*bug 268672,267084 modify by diganyun 2017/07/02 optimize red green brightness 0 end*/
	#ifdef LED_DEBUG_AW2015
...
{       
	...
	if (strnstr(saved_command_line, "androidboot.mode=ffbm-01",strlen(saved_command_line))){// mmi init the leds
		ffbm_flag = 1;
	}
