...
static u8 g_proximity_en = 0;
	...
	{
		#include "HLT_CCQ9100_T89239_C11_2017-10-10_1513.i"
	};
#endif
#ifdef MTK
#ifdef MTK_I2C_DMA
...
static int himax_input_register(struct himax_ts_data *ts)
{
	...
	set_bit(KEY_CUST_DBCLICK, ts->input_dev->keybit);
	set_bit(KEY_CUST_SWIPEUP, ts->input_dev->keybit);
	set_bit(KEY_CUST_02, ts->input_dev->keybit);
	set_bit(KEY_CUST_03, ts->input_dev->keybit);
	set_bit(KEY_CUST_04, ts->input_dev->keybit);
	set_bit(KEY_CUST_C, ts->input_dev->keybit);
	set_bit(KEY_CUST_Z, ts->input_dev->keybit);
	set_bit(KEY_CUST_07, ts->input_dev->keybit);
	set_bit(KEY_CUST_08, ts->input_dev->keybit);
	set_bit(KEY_CUST_S, ts->input_dev->keybit);
	set_bit(KEY_CUST_V, ts->input_dev->keybit);
	set_bit(KEY_CUST_W, ts->input_dev->keybit);
	set_bit(KEY_CUST_E, ts->input_dev->keybit);
...
#ifdef HX_HIGH_SENSE
static void himax_set_HSEN_enable(unsigned int enable)
{
	uint8_t cmd[4];
...
static irqreturn_t himax_ts_thread(int irq, void *ptr)
{
	...
	if (psensor_state) {	// not report the key
		buf[0] = HX_CMD_CLRES;
	...
	if (atomic_read(&ts->suspend_mode)&&(!FAKE_POWER_KEY_SEND)&&(ts->SMWP_enable)) {
		...
		switch (ret_event) {
			case EV_GESTURE_DBCLICK:
				I(" %s SMART WAKEUP KEY event: Gesture_DBClick\n",__func__);
				KEY_EVENT = KEY_CUST_DBCLICK;
			break;
			case EV_GESTURE_SWIPEUP:
				I(" %s SMART WAKEUP KEY event: Gesture_SWIPEUP\n",__func__);
				KEY_EVENT = KEY_CUST_SWIPEUP;
			break;
			case EV_GESTURE_02:
			...
			break;
			case EV_GESTURE_C:
				I(" %s SMART WAKEUP KEY event: Gesture_C\n",__func__);
				KEY_EVENT = KEY_CUST_C;
			break;
			case EV_GESTURE_Z:
				I(" %s SMART WAKEUP KEY event: Gesture_Z\n",__func__);
				KEY_EVENT = KEY_CUST_Z;
			break;
			case EV_GESTURE_07:
			...
			break;
			case EV_GESTURE_S:
				I(" %s SMART WAKEUP KEY event: Gesture_S\n",__func__);
				KEY_EVENT = KEY_CUST_S;
			break;
			case EV_GESTURE_V:
				I(" %s SMART WAKEUP KEY event: Gesture_V\n",__func__);
				KEY_EVENT = KEY_CUST_V;
			break;
			case EV_GESTURE_W:
				I(" %s SMART WAKEUP KEY event: Gesture_W\n",__func__);
				KEY_EVENT = KEY_CUST_W;
			break;
			case EV_GESTURE_E:
				I(" %s SMART WAKEUP KEY event: Gesture_E\n",__func__);
				KEY_EVENT = KEY_CUST_E;
			break;
			case EV_GESTURE_13:
		...
		if(ret_event)
			{
				...
				input_sync(private_ts->input_dev);
				FAKE_POWER_KEY_SEND=false;	// TT1007174 the gesture key report once only in suspend state!
...
int himax_ts_register_interrupt(struct i2c_client *client)
{
	...
	ts->irq_enabled = 0;
	ts->use_irq = 0;
	E("himax_ts_register_interrupt HX_INT_IS_EDGE=%d!\n", HX_INT_IS_EDGE);
	//Work functon
...
test_retry:
	...
	//msleep(10);
	/* write the holitech vendor Threshold value for open/short test */
	RB1H[0]= 0x00;
	RB1H[1]= 0x00;
	RB1H[2]= 0xD7;
	RB1H[3]= 0x32;
	...
	/* write the holitech vendor Threshold value open/short test end */
	msleep(10);
#else
	cmdbuf[0] = 0x06;
...
static DEFINE_MUTEX(himax_selftest_mutex);

static ssize_t himax_self_test_read_attr(struct device *dev,
                                   struct device_attribute *attr, char *buf)
{
	int val=0x00;
	int ret = 0;

	mutex_lock(&himax_selftest_mutex);
	...
	mutex_unlock(&himax_selftest_mutex);

	return ret;
}

static DEVICE_ATTR(hx_selftest, S_IRUGO, himax_self_test_read_attr, NULL);

/*add your attr in here*/
static struct attribute *himax_attributes[] = {
	&dev_attr_hx_selftest.attr,
	NULL
};
	...
	size_t len, loff_t *pos)
{
	...
	if(buf[0] == '0')
		...
		return -EINVAL;
	himax_set_HSEN_enable(ts->HSEN_enable);
	...
	size_t len, loff_t *pos)
{
	...
	ret = kstrtouint(buf, 0, &gesture_flag);

	I("%s, gesture_flag from user space=0x%x", __func__, gesture_flag);
	ts->gesture_cust_en[0] = (gesture_flag & 0x02)>>1;	// DB
	ts->gesture_cust_en[1] = (gesture_flag & 0x01);		// Swipe up

	/* gesture c z s v w e */
	if ((gesture_flag & 0x100)) {// gesture need bit8 to control it open or not
		ts->gesture_cust_en[10] = (gesture_flag & 0x04)>>2;
		ts->gesture_cust_en[6] = (gesture_flag & 0x08)>>3;
		ts->gesture_cust_en[5] = (gesture_flag & 0x10)>>4;
		ts->gesture_cust_en[12] = (gesture_flag & 0x20)>>5;
		ts->gesture_cust_en[9] = (gesture_flag & 0x40)>>6;
		ts->gesture_cust_en[11] = (gesture_flag & 0x80)>>7;
	} else {
		for(i=5; i < 16; i++){
	...
	if (ts->gesture_cust_en[0] || ts->gesture_cust_en[1]
		|| ts->gesture_cust_en[5] || ts->gesture_cust_en[6]
		|| ts->gesture_cust_en[9] || ts->gesture_cust_en[10]
		|| ts->gesture_cust_en[11] || ts->gesture_cust_en[12]) {
		ts->SMWP_enable = 1;
	} else {
		ts->SMWP_enable = 0;
	}
	...
	size_t len, loff_t *pos)
{
	...
	ret = kstrtouint(buf, 0, &psensor_state);
...
#ifdef HX_TP_PROC_SELF_TEST
	err = sysfs_create_group(&ts->client->dev.kobj, &himax_attribute_group);
	if (0 != err)
	{
		dev_err(&ts->client->dev, "%s() - ERROR: sysfs_create_group() failed.\n", __func__);
		sysfs_remove_group(&ts->client->dev.kobj, &himax_attribute_group);
		return -EIO;
	}
	else
	{
		pr_info("himax:%s() - sysfs_create_group() succeeded.\n",__func__);
	}
#endif
#endif
...
#if defined(CONFIG_FB)
static int himax_ts_resume_bythread(void *vts)
{
	struct himax_ts_data *ts = container_of((struct notifier_block *)vts, struct himax_ts_data, fb_notif);
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EARLY_EVENT_BLANK && ts &&
		...
		case FB_BLANK_UNBLANK:
			I("func=%s, line=%d resume!", __func__, __LINE__);
			resume_thread = kthread_run(himax_ts_resume_bythread, (void*)self, "himax_tp_resume_thread");
		break;
		...
		case FB_BLANK_NORMAL:
			I("func=%s, line=%d suspend!", __func__, __LINE__);
