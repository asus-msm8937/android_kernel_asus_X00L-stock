...
/*** add for virtual key report event, 09,18,2017, zlh ***/
#if CTP_HAVE_KEYS
    static u8 key_back_fingerid = 0;
    static u8 key_menu_fingerid = 0;
    static u8 key_back = 0;
    static u8 key_menu = 0;
#endif
/*********************************************************/
	buf_count_neg++;
...
/*** edit for virtual key report event, 09,18,2017, zlh ***/
#if CTP_HAVE_KEYS
            if(event->au16_y[i] > panel_y) {
                /* key_back event */
                if (event->au16_x[i] < panel_x_right) {
                    if(0 == key_back) {
                        input_report_key(data->input_dev, KEY_BACK, 1);
                    }
                    key_back = KEY_BACK;
                    /* get the finger id */
                    key_back_fingerid = event->au8_finger_id[i];
				}
                /* key_menu event */
				else if(event->au16_x[i] > panel_x_left) {
                    if(0 == key_menu) {
                        input_report_key(data->input_dev, KEY_MENU, 1);
                    }
                    key_menu = KEY_MENU;
                    /* get the finger id */
                    key_menu_fingerid = event->au8_finger_id[i];
				}
                /* out of the key_back area */
                else if (key_back && (key_back_fingerid == event->au8_finger_id[i])) {
					input_report_key(data->input_dev, KEY_BACK, 0);
                    key_back = 0;
                }
                /* out of the key_menu area */
                else if (key_menu && (key_menu_fingerid == event->au8_finger_id[i])) {
					input_report_key(data->input_dev, KEY_MENU, 0);
                    key_menu = 0;
                }
            }

            /* on the top of the virturl key area */
            else {
                /* out of the key_back area */
                if (key_back && (key_back_fingerid == event->au8_finger_id[i])) {
                    input_report_key(data->input_dev, KEY_BACK, 0);
                    key_back = 0;
                }
                /* out of the key_menu area */
...
/*** edit for BTN_TOUCH report event, 09,18,2017, zlh ***/
	if(0 == touchs) {
...
#if FTS_GLOVE_SURPORT
	if (1 == fts_glove_enabled) {
		err = fts_write_reg(fts_i2c_client, 0xC0, 1);// open glove function.
		if (err < 0)
		{
			pr_err("send glove config failed.");
		}
...
//Start:Reqxxx,liuyang3,wt,Modify,for add gesture switch.
	...
	if (fts_gesture_enabled)
	{
		  ...
		  pr_info("%s fts:enter gesture mode.\n", __func__);
...
static int fts_ts_resume_bythread(void* vts)
{
	struct fts_ts_data *fts_data = container_of((struct notifier_block *)vts, struct fts_ts_data, fb_notif);
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EARLY_EVENT_BLANK &&
		...
		if (*blank == FB_BLANK_UNBLANK)
			thread = kthread_run(fts_ts_resume_bythread, (void *)self, "fts_ts_resume_thread");
...
//End:Reqxxx,liuyang3.wt,ADD,20160521, for glove mode switch.	
	...
	reg_value=0;
	fts_read_reg(client,FTS_REG_FW_VER,&reg_value);
	sprintf(TP_info,"ft5436_v%04x",reg_value);
#endif	
