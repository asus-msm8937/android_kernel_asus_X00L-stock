...
#if FTS_SELF_TEST 
#define FTXXXX_INI_FILEPATH "/etc/"
#define FTS_OPENTEST_SAVE_DATA_PATH   "/mnt/sdcard/"
static DEFINE_MUTEX(fts_ctptest_mutex);
...
static int ftxxxx_SaveTestData(char *file_name, char *data_buf, int iLen)
{
	...
	memset(filepath, 0, sizeof(filepath));
	sprintf(filepath, "%s%s", FTS_OPENTEST_SAVE_DATA_PATH, file_name);
	if (NULL == pfile)
...
static int ftxxxx_get_testparam_from_ini(char *config_name)
{
	...
	filedata = kzalloc(inisize + 1, GFP_ATOMIC);
...
static ssize_t ftxxxx_ctptest_show(struct device *dev,
                                   struct device_attribute *attr, char *buf)
{
  /* place holder for future use */
    char cfgname[128];
	char *testdata = NULL;
	int iTestDataLen=0,count=0;
	u8 vid = 0;

	mutex_lock(&fts_ctptest_mutex);

#ifdef FT_ESD_PROTECT
       esd_switch(0);
#endif   

#if FTS_AUTO_UPGRADE   //make sure ctp test after fw upgraded completely
	if (fw_upgrade_status==FW_UPGRADING)
	{
		for(;fw_upgrade_status==FW_UPGRADING;){
			
			if ((vid%4) == 0)
				FTS_DBG("tp firmware upgrading status:%d\n",fw_upgrade_status);
				msleep(200);
				vid++;
			if (vid >30)   //6s
			{
				FTS_DBG("takes 6s tp still upgrading status:%d\n",fw_upgrade_status);
...
#endif
	testdata = kzalloc(1024*80, GFP_ATOMIC);
	if(NULL == testdata)
	{
		FTS_DBG("kmalloc failed in function:%s\n", __func__);
		return -1;
	}

	memset(cfgname, 0, sizeof(cfgname));
	fts_read_reg(fts_i2c_client,FTS_REG_FW_VENDOR_ID,&vid);
	FTS_DBG("ft5x46_ctptest_show vid=0x%x\n",vid);
	if (vid == OFILM_VENDOR){
		 sprintf(cfgname, "%s", "fts_ofilm.ini");
	 }else if (vid == EACH_VENDOR){ 	
		 sprintf(cfgname, "%s", "fts_each.ini");
	 }else{
			 pr_err("ctp test not found test config \n");
    }	 

	cfgname[strlen(cfgname)] = '\0';
	...
	if(ftxxxx_get_testparam_from_ini(cfgname) <0)
		FTS_DBG("get testparam from ini failure file:%s\n",cfgname);
	else {		
	...
	mutex_unlock(&fts_ctptest_mutex);
...
static DEVICE_ATTR(ftsctptest, S_IRUGO | S_IWUSR, ftxxxx_ctptest_show, ft5x46_ctptest_store);
#endif
