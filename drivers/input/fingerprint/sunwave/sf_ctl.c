 ...
 */
#define SF_DRV_VERSION "v0.9.1-20161227"
...
static int sf_ctl_device_pwr_off(void)
{
    int err = 0;
    xprintk(KERN_DEBUG, "%s(..) enter.\n", __FUNCTION__);
...
static int sf_ctl_init_gpio_pins(void)
{
    ...
    struct device_node *dev_node = NULL;
    xprintk(KERN_DEBUG, "%s(..) enter.\n", __FUNCTION__);
    dev_node = of_find_compatible_node(NULL, NULL, "goodix,fingerprint");
    ...
    sf_ctl_dev.pwr_num = of_get_named_gpio(dev_node, "goodix,gpio_pwr", 0);
    sf_ctl_dev.reset_num = of_get_named_gpio(dev_node, "goodix,gpio_reset", 0);
    sf_ctl_dev.irq_num = of_get_named_gpio(dev_node, "goodix,gpio_irq", 0);
